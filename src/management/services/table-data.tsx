...
export default function ListTable({ data }: ListTableProps) {
	...
	function handleNavigation(id: string) {
		router(`/management/services/${id}`);
	}

	const handleSearchRequest = useCallback(async () => {
		const services = await searchPOSServicesTable<ServicesManagement>(
			debouncedValue,
			"services",
	...
	function handleWidthChange(width: number) {
		if (width < 768) {
			...
			table.getColumn("unit")?.toggleVisibility(true);
			table.getColumn("price")?.toggleVisibility(true);
		}
	}
	useLayoutEffect(() => {
